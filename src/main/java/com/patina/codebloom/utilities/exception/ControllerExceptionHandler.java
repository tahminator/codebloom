...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<ApiResponder<?>> handleResponseStatusException(
        final ResponseStatusException ex
    ) {
        return ResponseEntity.status(ex.getStatusCode()).body(
            ApiResponder.failure(ex.getReason())
        );
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<ApiResponder<?>> handleMethodArgumentNotValid(
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
            ApiResponder.failure(String.join(", ", errors))
        );
    }
    ...
    public ResponseEntity<ApiResponder<?>> handleMethodArgumentNotValid(
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
            ApiResponder.failure(ex.getMessage())
        );
    }
    ...
    public ResponseEntity<ApiResponder<?>> handleThrowable(final Throwable rx) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
            ApiResponder.failure(rx.getMessage())
        );
    }
}
