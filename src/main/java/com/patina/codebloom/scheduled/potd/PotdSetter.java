...
public class PotdSetter {
    ...
    public void setPotd() {
        com.patina.codebloom.common.leetcode.models.POTD leetcodePotd =
        ...
        if (
            currentPotd != null &&
            currentPotd.getTitle().equals(leetcodePotd.getTitle())
        ) {
            // It's already the latest POTD, don't want to do it again.
            LOGGER.info(
                "POTD has already been set before, will not be doing it again."
            );
            return;
        }
        ...
        potdRepository.createPOTD(
            POTD.builder()
                ...
                .multiplier(
                    ScoreCalculator.calculateMultiplier(
                        (leetcodePotd.getDifficulty())
                    )
                )
