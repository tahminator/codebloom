...
public class AuthController {
    ...
    public ResponseEntity<ApiResponder<AuthenticationObjectDto>> validateAuth(
        ...
        return ResponseEntity.ok().body(
            ApiResponder.success(
                ...
                AuthenticationObjectDto.fromAuthenticationObject(
    ...
    public RedirectView logout(
        ...
        try {
            ...
            boolean sessionDeleted = sessionRepository.deleteSessionById(
                session.getId()
            );

            if (!sessionDeleted) {
                return new RedirectView(
                    "/login?success=false&message=You are not logged in."
                );
            }

            ResponseCookie strippedCookie = ResponseCookie.from(
                "session_token",
                ""
            )
                .path("/")
            ...
            response.addHeader(
                ...
                strippedCookie.toString()
            );

            return new RedirectView(
                "/login?success=true&message=You have been logged out!"
            );
        } catch (Exception e) {
            return new RedirectView(
                "/login?success=false&message=You are not logged in."
    ...
    public ResponseEntity<ApiResponder<Empty>> enrollSchool(
        @Valid @RequestBody final EmailBody emailBody,
        final HttpServletRequest request
    ) {
        ...
        if (!supportedDomains.contains(domain)) {
            ...
            throw new ResponseStatusException(
                ...
                "The email is not part of our supported schools domains: " +
                    supportedSchools
            );
        }
        ...
        try {
            ...
            emailClient.sendMessage(
                SendEmailOptions.builder()
                    ...
                    .body(
                        reactEmailClient.schoolEmailTemplate(verificationLink)
                    )
                    .build()
            );
            return ResponseEntity.ok().body(
                ApiResponder.success(
                    "Magic link sent! Check your school inbox to continue.",
                    Empty.of()
                )
            );
    ...
    public RedirectView verifySchoolEmail(final HttpServletRequest request) {
        ...
        if (session == null) {
            return new RedirectView(
                "/settings?success=false&message=You are not logged in"
            );
        }
        ...
        try {
            ...
            return new RedirectView(
                "/settings?success=false&message=Invalid or expired token"
            );
        }
        ...
        if (!magicLinkId.equals(currentUserId)) {
            return new RedirectView(
                "/settings?success=false&message=ID does not match current user"
            );
        }
        ...
        if (!isSuccessful) {
            throw new RuntimeException(
                "User repository failed to update user and add school email."
            );
        }
        ...
        if (schoolEnum == null) {
            return new RedirectView(
                "/settings?success=false&message=This email is not supported"
            );
        }
