...
public class DuelController {
    ...
    private void validatePlayerNotInLobby(final String playerId) {
        var availableLobby =
            lobbyRepository.findAvailableLobbyByLobbyPlayerPlayerId(playerId);
        ...
        var activeLobby = lobbyRepository.findActiveLobbyByLobbyPlayerPlayerId(
            playerId
        );
    ...
    @Operation(
        summary = "Join lobby",
        description = "Join a lobby by providing the lobby code."
    )
    @ApiResponses(
        value = {
            @ApiResponse(
            ...
            @ApiResponse(
                responseCode = "200",
                description = "Lobby has been successfully joined!"
            ),
        }
    )
    ...
    public ResponseEntity<ApiResponder<Empty>> joinLobby(
        ...
        var lobby = lobbyRepository
            ...
            .orElseThrow(() ->
                new ResponseStatusException(
                    ...
                    "The party with the given code cannot be found."
                )
            );
        ...
        return ResponseEntity.ok(
            ApiResponder.success("Party successfully joined!", Empty.of())
        );
    }
    ...
    @ApiResponses(
        value = {
            ...
            @ApiResponse(
                responseCode = "200",
                description = "Lobby has been successfully started!"
            ),
        }
    )
    ...
    public ResponseEntity<ApiResponder<Empty>> startLobby(
        ...
        if (lobby.getPlayerCount() < 2) {
            throw new ResponseStatusException(
                HttpStatus.BAD_REQUEST,
                "You must have at least 2 players!"
            );
        }
    ...
    public ResponseEntity<ApiResponder<Empty>> leaveParty(
        ...
        return ResponseEntity.ok(
            ApiResponder.success("Successfully left the lobby.", Empty.of())
        );
    }

    @Operation(
        summary = "Create party",
        description = "Create a new lobby and become the host"
    )
    @ApiResponse(
        responseCode = "200",
    ...
    public ResponseEntity<ApiResponder<Empty>> createParty(
        ...
        var existingLobbyPlayer =
            lobbyPlayerRepository.findLobbyPlayerByPlayerId(playerId);
        ...
        OffsetDateTime expiresAt = StandardizedOffsetDateTime.now().plusMinutes(
            30
        );
        ...
        return ResponseEntity.ok(
            ApiResponder.success(
                "Lobby created successfully! Share the join code: " +
                    lobby.getJoinCode(),
                Empty.of()
            )
        );
    ...
    @Operation(
        ...
        description = "Submit a question for the current duel"
    )
    @ApiResponse(
        responseCode = "200",
        description = "Question has been successfully submitted!"
    )
    ...
    public ResponseEntity<ApiResponder<Empty>> submitQuestion(
        ...
        recentSubmissions = throttledLeetcodeClient.findSubmissionsByUsername(
            user.getLeetcodeUsername()
        );
        int start = Math.max(0, recentSubmissions.size() - 5);
        recentSubmissions = recentSubmissions.subList(
            start,
            recentSubmissions.size()
        );

        List<LobbyQuestion> lobbyQuestions =
            lobbyQuestionRepository.findLobbyQuestionsByLobbyId(lobby.getId());

        if (lobbyQuestions.isEmpty()) {
            throw new ResponseStatusException(
                HttpStatus.NOT_FOUND,
                "No questions assigned to this lobby"
            );
        }
        ...
        for (LeetcodeSubmission submission : recentSubmissions) {
            for (LobbyQuestion lobbyQuestion : lobbyQuestions) {
                QuestionBank questionBank =
                    questionBankRepository.getQuestionById(
                        lobbyQuestion.getQuestionBankId()
                    );
                ...
                if (
                    ...
                    submission
                        .getTitleSlug()
                        .equals(questionBank.getQuestionSlug())
                ) {
                    ...
                    boolean alreadySubmitted = existingPlayerQuestions
                        .stream()
                        .anyMatch(
                            question ->
                                question.getQuestionId().isPresent() &&
                                question
                                    .getQuestionId()
                                    .get()
                                    .equals(questionBank.getId())
                        );
        ...
        float multiplier = ScoreCalculator.calculateMultiplier(
            matchedQuestionBank.getQuestionDifficulty()
        );
        int points = ScoreCalculator.calculateScore(
            matchedQuestionBank.getQuestionDifficulty(),
            matchedQuestionBank.getAcceptanceRate(),
            multiplier
        );
        ...
        matchedLobbyQuestion.setUserSolvedCount(
            matchedLobbyQuestion.getUserSolvedCount() + 1
        );

        boolean lobbyQuestionUpdated =
        ...
        boolean isPlayerUpdated = lobbyPlayerRepository.updateLobbyPlayer(
            lobbyPlayer
        );
        if (!isPlayerUpdated) {
            throw new ResponseStatusException(
    ...
    @Operation(
        summary = "SSE endpoint for duel data",
        description = """
    ...
    public SseWrapper<ApiResponder<DuelData>> getDuelData(
        ...
        SseWrapper<ApiResponder<DuelData>> emitter = new SseWrapper<>(
            1_800_000L
        );
        try {
