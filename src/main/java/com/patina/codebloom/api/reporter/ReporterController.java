...
public class ReporterController {
    ...
    @PostMapping("/error")
    public ResponseEntity<ApiResponder<Empty>> ingestErrors(final @RequestBody IngestErrorsBody ingestErrorBody, final HttpServletRequest request) {
    ...
    @PostMapping("/log")
    public ResponseEntity<ApiResponder<Empty>> ingestLog(final @RequestBody IngestLogBody ingestLogBody, final HttpServletRequest request) {
        if (!validateOrigin(request)) {
            // don't let the bad actor know it failed.
            return ResponseEntity.ok(ApiResponder.success("ok", Empty.of()));
        }
