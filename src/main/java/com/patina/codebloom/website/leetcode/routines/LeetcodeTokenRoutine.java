...
/**
 * This routine will programmatically "steal" an
 * authentication token from Leetcode via GitHub OAuth. This
 * is required because of the fact that in order to access
 * certain GraphQL routes (such as submitted code and POTD),
 * you must be authenticated.
 * 
 */
@Component
public class LeetcodeTokenRoutine {
    ...
    public LeetcodeTokenRoutine(final AuthRepository authRepository, final Email email) {
     ...
     * <div /> This function utilizes Playwright in order to get
     * an authentication key from Leetcode. That code is stored
     * in the database and can then be used to run authenticated
     * queries such as used to retrieve code from our user
     * submissions.
     */
    ...
    public void stealAuthCookie() {
        ...
        try (Playwright playwright = Playwright.create()) {
            ...
            if (page.isVisible("#device-verification-prompt") || page.isVisible("#session-otp-input-description")) {
                ...
                Matcher matcher = pattern.matcher(target.getMessage());
            ...
            if (page.isVisible("button[name=\"authorize\"]")) {
                ...
                if (buttons.size() > 1) {
                    // Click the second button (which is the actual authorize
                    // button)
                    buttons.get(1).click();
                }
            }
                ...
                for (Cookie cookie : page.context().cookies()) {
                    if (cookie.name.equals("LEETCODE_SESSION")) {
                        ...
                        try {
                            ...
                            LeetcodeTokenRoutine.cookie = cookie.value;
                        } catch (Exception e) {
