...
public class UserSqlRepository implements UserRepository {
    ...
    public User createNewUser(final User user) {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            // User cannot be instantiated with a leetcodeUsername, it
            // gets collected after
            // user authentication.
            stmt.setNull(4, java.sql.Types.VARCHAR);
            stmt.setString(5, user.getNickname());

            // We don't care what this actually returns, it can never be
            // more than 1 anyways
            // because id is UNIQUE. Just return the new user every time
            // if we want to do
            // any work on it.
            stmt.executeUpdate();
    ...
    @Override
    public PrivateUser _getPrivateUserById(final String inputId) {
