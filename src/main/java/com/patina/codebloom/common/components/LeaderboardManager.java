...
public class LeaderboardManager {
    ...
    private int maxWinners(final int size) {
        return Math.min(
            Math.max(size, MIN_POSSIBLE_WINNERS),
            MAX_POSSIBLE_WINNERS
        );
    }
    ...
    private String calculatePlaceString(final int place) {
        return switch (place) {
            ...
            default -> throw new IllegalArgumentException(
                "Can only process 1st, 2nd, and 3rd place"
            );
        };
    }
    ...
    public void generateAchievementsForAllWinners() {
        ...
        for (var pair : filterOptsAndTags) {
            ...
            List<Indexed<UserWithScore>> winners = users.subList(
                0,
                maxWinners(users.size())
            );

            for (int i = 0; i < winners.size(); i++) {
                int place = i + 1;
                log.info(
                    "on leaderboard for {} for winner #{}",
                    pair.getRight().getResolvedName(),
                    place
                );
    ...
    public int getLeaderboardUserCountById(
        ...
        return leaderboardRepository.getLeaderboardUserCountById(
            leaderboardId,
            options
        );
    }
    ...
    public Page<Indexed<UserWithScoreDto>> getLeaderboardUsers(
        final String currentLeaderboardId,
        final LeaderboardFilterOptions options,
        ...
        int totalUsers = leaderboardRepository.getLeaderboardUserCountById(
        ...
        List<Indexed<UserWithScoreDto>> indexedUserWithScoreDtos =
            leaderboardData
                .stream()
                .map(indexed ->
                    indexed.map(UserWithScoreDto::fromUserWithScore)
                )
                .toList();

        Page<Indexed<UserWithScoreDto>> createdPage = new Page<>(
            hasNextPage,
            indexedUserWithScoreDtos,
            totalPages,
            parsedPageSize
        );
        return createdPage;
