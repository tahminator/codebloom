...
public class AdminControllerTest {
    ...
    private final LeaderboardRepository leaderboardRepository = mock(
        LeaderboardRepository.class
    );
    private final AnnouncementRepository announcementRepository = mock(
        AnnouncementRepository.class
    );
    private final QuestionRepository questionRepository = mock(
        QuestionRepository.class
    );
    private final Protector protector = mock(Protector.class);
    private final DiscordClubManager discordClubManager = mock(
        DiscordClubManager.class
    );
    private final LeaderboardManager leaderboardManager = mock(
    ...
    void testCreateLeaderboardSuccessNoExistingLeaderboard() {
        ...
        ResponseEntity<ApiResponder<Empty>> response =
            adminController.createLeaderboard(request, body);
        ...
        verify(
            discordClubManager,
            never()
        ).sendLeaderboardCompletedDiscordMessageToAllClubs();
        verify(leaderboardManager, never()).generateAchievementsForAllWinners();
        verify(leaderboardRepository, never()).disableLeaderboardById(
            anyString()
        );
    }
    ...
    void testCreateLeaderboardEmptyName() {
        ...
        verify(leaderboardRepository, never()).addNewLeaderboard(
            any(Leaderboard.class)
        );
    }
    ...
    void testCreateLeaderboardNameWithLeadingAndTrailingSpaces() {
        ...
        verify(protector).validateAdminSession(request);
        verify(leaderboardRepository).getRecentLeaderboardMetadata();
        verify(leaderboardRepository).addNewLeaderboard(
            argThat(leaderboard ->
                "Challenge 2024".equals(leaderboard.getName())
            )
        );
        verify(leaderboardRepository).addAllUsersToLeaderboard(any());
    }
